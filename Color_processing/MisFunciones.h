...
cv::Mat segmentacion_HSV(string nombre_ventana, cv::Mat imagen, cv::Mat &mascara){
	
		cv::Mat imagen_en_grayscale;
		cv::cvtColor(imagen, imagen_en_grayscale, CV_RGB2GRAY);
		
		//Un rojo claro u oscuro, van a compartir el valor de H, lo que los diferenciará serán los valores de
		...
		cv::cvtColor(croppedImage, croppedImage_en_HSV, CV_RGB2HSV);
		...
		//Valores - determinados "experimentalmente" - aproximación de lo que se ve en el histograma
