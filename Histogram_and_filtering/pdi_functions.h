...
namespace pdi{
	...
	inline cv::Mat mosaic( const std::vector<cv::Mat> &images, size_t r){
		...
		// opencv Rect class -> to create a rectangle
		...
		for(size_t K=0; K<images.size(); ++K){
			...
			images[K].copyTo(region); //Copies the matrix to another one
		}
...
//		const std::complex<float> I{0,1};  //Investigar la clase complex
		const std::complex<float> I;
